...
| Phase 3 — Sensors & sleep states | M3.1–M3.3 | 🔨 in progress (M3.1 done) |
...
### M3.3 — Fusion state machine — 🔨 in progress (slice 1: fusion core + replay gates ✓)
...
- [ ] [A] Session integrity: count + boundaries ±2 min (slice 1 ✓ via back-dated boundaries); **survives engine restart** (slice 2 — needs the live persisted worker)
- [ ] [A] Playwright: Tonight timeline renders replayed night; scrub-to-clip works (slice 3)
