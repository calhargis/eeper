...
| Phase 4 — Trends & pulse-ox | M4.1–M4.3 | 🔨 in progress (M4.1 [AUTO] done) |
...
### M4.2 — Pulse-ox (optional, gated) — 🔨 in progress (slice 1: gating + disclaimer + copy lint ✓)
...
- [ ] [A] Low-quality samples discarded, not stored/fused; discard rate observable (slice 2 — the ingestion path)
- [~] [A] Copy lint on pulse-ox strings; accuracy caveat asserted on every view (slice 1: the **clinical-terms copy lint** now exists and gates CI — user-facing copy may make no medical/diagnostic/vital-sign/alarm claim, with the reviewed disclaimer as the sole exemption, making CONTRIBUTING's claim real; the "caveat on every view" Playwright is slice 3)
- [ ] [A] Fusion consumes HR features only from quality-gated samples (slice 2)
