...
### M6.1 — Thermal capture node & publisher — 🔨 in progress (slice 1: node contract + publisher logic ✓)

- [x] [A] Grid + features messages validate against §4.5 schema; malformed frames dropped (slice 1: `ThermalGridMessage` / `ThermalFeaturesMessage` reject a truncated grid, non-finite / out-of-range temps, unknown fields, and a non-normalized centroid; the `ThermalPublisher` drops a failed or malformed read and never emits a bad or stale grid — built via pydantic so what reaches the wire always validates)
- [ ] [A] Pairing/revocation parity through M3.1 flow; ACL + reconcile tests green with thermal class (slice 2)
- [~] [A] I²C failure simulation: health degrades, no crash, auto-recovery (slice 1: the publisher drops `None`/malformed reads, tracks read-failure / dropped-invalid / fail-streak stats, degrades the `quality` field on the recovered frame, and resumes automatically — no crash; the server-side device-health flip within the heartbeat window lands with the ingestor in slice 2)
- [x] [A] Publisher rate capped at 4 Hz (slice 1: `tick()` holds a 4 Hz grid cap regardless of call rate)
