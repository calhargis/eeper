...
### M4.2 — Pulse-ox (optional, gated) — 🔨 in progress (slices 1–2: gating + copy lint ✓, quality-gated ingestion + fusion HR ✓)
...
- [~] [A] Copy lint on pulse-ox strings; accuracy caveat asserted on every view (slice 1: the **clinical-terms copy lint** exists + gates CI, reviewed disclaimer the sole exemption; the "caveat on every view" Playwright is slice 3)
