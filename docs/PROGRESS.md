...
| Phase 4 — Trends & pulse-ox | M4.1–M4.3 | 🔨 in progress (M4.1 slice 1) |
...
### M4.1 — Trends — 🔨 in progress (slices 1–2: aggregates + compression ✓, API + CSV ✓)
...
- [ ] [A] Playwright: charts render; CSV export matches; viewer denied export (slice 2 ✓ server-side: CSV export matches `/trends/nightly` and a viewer role is denied export, tested against a real TimescaleDB; the charts-render Playwright is slice 3)
