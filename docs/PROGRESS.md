...
### M6.1 — Thermal capture node & publisher — ✅ [AUTO] done (slices 1–3: node contract + publisher ✓, server ingestion + pairing parity ✓, MLX90640 hardware entrypoint + docs ✓); [MANUAL] 24 h bench pending (hardware)

- [x] [A] Grid + features messages validate against §4.5 schema; malformed frames dropped (slice 1: `ThermalGridMessage` / `ThermalFeaturesMessage` reject a truncated grid, non-finite / out-of-range temps, unknown fields, and a non-normalized centroid; the `ThermalPublisher` drops a failed or malformed read and never emits a bad or stale grid — built via pydantic. Slice 2: the `ThermalIngestor` re-validates against the same contract and drops malformed messages; the M3.1 sensor ingestor skips the richer thermal topics)
- [x] [A] Pairing/revocation parity through M3.1 flow; ACL + reconcile tests green with thermal class (slice 2: `thermal` is a device kind, so a thermal node pairs / publishes / unpairs / revokes through the exact M3.1 flow with no special-casing; a sensors integration test confirms features land in `thermal_features`, the node reads online, ACL isolation from other devices' subtrees holds, and unpair revokes the credential — validated end-to-end against a real broker)
- [x] [A] I²C failure simulation: health degrades, no crash, auto-recovery (slice 1: the publisher drops `None`/malformed reads, tracks read-failure / dropped-invalid / fail-streak stats, degrades the `quality` field on the recovered frame, and resumes automatically — no crash; slice 2: because the node stops publishing on a failure, the server's `last_seen_at` goes stale and the device flips offline within the heartbeat window, recovering when features resume — the standard M3.1 health path)
- [x] [A] Publisher rate capped at 4 Hz (slice 1: `tick()` holds a 4 Hz grid cap regardless of call rate; slice 2 adds the §4.5 low-rate features cadence, decoupled from the grid rate)
- [ ] [M] 24 h physical stream, no I²C lockup; FOV presence visibly registers — ______ (the MLX90640 hardware entrypoint `python -m eeper.thermal` shipped in slice 3; this is the physical bench run)
...
| 2026-07-14 | M6.1 slice 2 (thermal server ingestion + M3.1 pairing parity) — **completes M6.1 [AUTO]**. The server side of the thermal input. Adds a **`thermal` device kind** (so pairing goes through the exact M3.1 flow — dynsec credential + per-device ACL — with no special-casing), a **`thermal_features` hypertable** (only the low-rate derived features are stored; the raw grid is never persisted in prod), and a **`ThermalIngestor`** (mirrors the M3.1 sensor / M4.2 pulse-ox ingestors: a paho thread validates each `eeper/dev/+/thermal_features` message against the §4.5 contract + enqueues; an async task batch-writes to `thermal_features` and advances the device's `last_seen_at` for health). The M3.1 sensor ingestor now skips `thermal` / `thermal_features` (richer contract), and the publisher gains the §4.5 low-rate features cadence (decoupled from the ≤4 Hz grid rate). Wired into the api lifespan. Tests: pure ingestor validation/drop/topic + the sensor-ingestor-skips-thermal guard; a real-DB test that features are stored and device health advances (and an unknown device is ignored); and a **sensors integration test** that a thermal node pairs, publishes features that land in `thermal_features`, reads online, is ACL-isolated from other devices' subtrees, and revokes on unpair — validated end-to-end against a real broker locally (259 server tests green). Remaining M6.1: the MLX90640 hardware entrypoint + node docs (slice 3) for the [MANUAL] 24 h bench. |
