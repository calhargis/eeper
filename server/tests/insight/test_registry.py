...
def test_camera_with_audio_reports_motion_and_audio_level() -> None:
    assert _names(has_audio=True) == {"motion", "audio_level"}


def test_extractor_statuses_and_requirements() -> None:
    by_name = {e.name: e for e in REGISTRY}
    assert by_name["motion"].status == "active"
    assert by_name["audio_level"].status == "declared"
    assert by_name["motion"].requires == frozenset({Modality.VIDEO})
    assert by_name["audio_level"].requires == frozenset({Modality.AUDIO})
