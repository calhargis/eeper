...
### M2.4 — Events, clips, nudges — 🔨 server implemented (in review; web half next). CI: `stack`/`recorder` + `python` jobs
...
delivery policy (quiet hours, per-camera rate-limit) lives in the worker.
...
- [ ] [A] Playwright: event appears in Tonight v0 via WebSocket; clip plays — **web PR**
